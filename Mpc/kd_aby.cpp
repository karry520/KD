...
void init_aby(std::string address, uint16_t port, bool role_is_server) {
    ...
    party = new ABYParty(role, address, port, LT, bitlen, nthreads, mt_alg);
    std::cout << "init first success!" << std::endl;
...
void init1_aby(std::string address, uint16_t port, bool role_is_server) {
    ...
    party1 = new ABYParty(role, address, port, LT, gbitlen, nthreads, mt_alg);
    std::cout << "init second success!" << std::endl;
}

void shutdown_aby() {
    delete party;
    delete party1;
    std::cout << "shutdown successs!" << std::endl;
}
...
std::vector<uint8_t> kd_sru(bool role, std::vector<uint8_t> *idx, uint32_t num_workers, uint32_t length, int f) {
    ...
    ArithmeticCircuit *ac = (ArithmeticCircuit *) sharings[2]->GetCircuitBuildRoutine();
    BooleanCircuit *yc = (BooleanCircuit *) sharings[1]->GetCircuitBuildRoutine();
    BooleanCircuit *bc = (BooleanCircuit *) sharings[0]->GetCircuitBuildRoutine();
...
kd_top(bool role, std::vector<uint32_t> *grad, std::vector<uint32_t> *fgrad, uint32_t k, uint32_t num_workers, uint32_t f) {
    ...
    std::vector<Sharing *> &sharings = party1->GetSharings();
